...
def user_can_ban(mystic):
    ...
    async def wrapper(app : Client, message : Message,*args,**kwargs):
        ...
        if (user.privileges and not user.privileges.can_restrict_members) and user_id not in SUDO: 
...
def user_can_del(mystic):
    ...
    async def wrapper(app : Client, message : Message,*args,**kwargs):
        ...
        if (user.status in COMMANDERS and not user.privileges.can_delete_messages) and user_id not in SUDO:                     
...
def user_can_change_info(mystic):
    ...
    async def wrapper(app : Client, message : Message,*args,**kwargs):
        ...
        if (user.status in COMMANDERS and not user.privileges.can_change_info) and user_id not in SUDO:                     
...
def user_can_promote(mystic):
    ...
    async def wrapper(app : Client, message : Message,*args,**kwargs):
        ...
        if (user.status in COMMANDERS and not user.privileges.can_promote_members) and user_id not in SUDO:                     
